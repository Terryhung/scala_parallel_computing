...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    val points = input.slice(from, until)
    var local_max: Float = 0
    var head = from

    for (num <- points) {
      local_max = max(num/head, local_max)
      head = head + 1
    }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold){
      ...
      val (r, l) = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
      Node(r, l)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var head = from
    var local_max = startingAngle
    while (head < until){
      local_max = max(local_max, input(head)/head)
      output(head) = local_max
      head = head + 1
    }
  }
